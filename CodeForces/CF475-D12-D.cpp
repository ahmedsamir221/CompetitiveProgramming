/*
   if we start from every index and tried to find the first position that will change the GCD from our current
   position to it , then we will iterate at most (log 1e9) .
   we can find that position in (log 3e5 ) using persistent segment tree .
   another way we can use binary search with sparse table .
*/
