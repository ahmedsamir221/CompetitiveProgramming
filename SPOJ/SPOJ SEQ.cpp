...
input
        ...
        while(t--)
        {
            ...
            {
                matrix init(k,row(k,0)) ,trans(k,row(k,0));
                forr(i,1,k)init[0][i-1]=b[i];
                forr(i,1,k-1)trans[i][i-1]=1;
                forr(i,0,k-1)trans[i][k-1]=c[k-i];
                matrix ans=mul(init,matPow(trans,n-k,1e9),1e9);
                cout<<ans[0][k-1]<<endl;
            }
